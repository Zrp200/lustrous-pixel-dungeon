...
public class TitleScene extends PixelScene {
	...
	/*	TitleButton btnSupport = new TitleButton(Messages.get(this, "support")){
		...
		int GAP = (int)(h - topRegion - (LustSettings.landscape() ? 3 : 4)*BTN_HEIGHT)/3;
		GAP /= LustSettings.landscape() ? 3 : 4;
		GAP = Math.max(GAP, 2);
	...
	private static class TitleButton extends StyledButton {
		...
		TitleButton(String label){
			this(label, 9);
		}
		
		TitleButton(String label, int size){
