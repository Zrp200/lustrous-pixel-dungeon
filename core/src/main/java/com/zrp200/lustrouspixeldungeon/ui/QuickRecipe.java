...
import com.zrp200.lustrouspixeldungeon.items.potions.elixirs.ElixirOfArcaneArmor;
...
import com.zrp200.lustrouspixeldungeon.items.spells.WildEnergy;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					LustrousPixelDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(LustrousPixelDungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				LustrousPixelDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene)LustrousPixelDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
		...
		switch (pageIdx){
			case 0: default:
				result.add(new QuickRecipe( new Potion.SeedToPotion(), new ArrayList<>(Arrays.asList(new Plant.Seed.PlaceHolder().quantity(3))), new WndBag.Placeholder(ItemSpriteSheet.POTION_HOLDER){
					{
						trueName = Messages.get(Potion.SeedToPotion.class, "name");
					}
			...
			case 2:
				...
				result.add(new QuickRecipe( new Blandfruit.CookFruit(),
						...
						new Blandfruit(){
							{
								trueName = Messages.get(Blandfruit.class, "cooked");
							}
